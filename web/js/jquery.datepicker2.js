	...
	$.datePicker = {
		defaults: {
			...
			dateParse: function(string) {
				return $.datePicker.api.date(string,0);
			},
            dateParsemy: function(string) {
                return $.datePicker.api.date(string,1);
            }
		},
		api: {
			date: function(string,typ) {
				var date = null;
			...
			createWidget: function(options) {
				...
				widget.views.month.on('click', '.header-title1', function(e) {
					var el = $(this);
					e.preventDefault();
                ...
                widget.views.month.on('click', '.header-title2', function(e) {
                    var el = $(this);
                    e.preventDefault();
                    if ( opts.callbacks.onViewDecade(widget, opts.views.decade.show) ) {
                        opts.animate(widget.views.month, 'fadeOut', function() {
			...
			createMonthView: function(opts, box) {
				var box = box || $('<div class="datepicker-box"></div>'),
					...
					title = '<div class="header-title1">'+opts.strings.months[ settings.show.getMonth() ] + '</div> <div class="header-title2">' + settings.show.getFullYear()+'</div>',
					header =  $.datePicker.api.createHeader(title, opts),
	...
	jQuery(document).ready(function($) {
		$('[data-select=datepicker]').each(function() {
			...
			input.on('click', function() {
				var val = input.val();
				var date = val ? $.datePicker.defaults.dateParsemy(val) : null;
